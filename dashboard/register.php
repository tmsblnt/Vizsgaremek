...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Az adatbázisba beszúrás
    $sql = "INSERT INTO felhasznalok (felhasznalonev, jelszo) VALUES (?, ?)";

    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param("ss", $felhasznalonev, $jelszo_hash);

        if ($stmt->execute()) {
            $message = "<div class='success-message'><i class='fa-solid fa-circle-check'></i> A regisztráció sikeres!</div>";
            // Átirányítás a bejelentkezési oldalra 3 másodperc után
            header("refresh:3;url=login.php");
        } else {
            $message = "<div class='error-message'><i class='fa-solid fa-circle-exclamation'></i> Hiba történt a regisztráció során: " . $stmt->error . "</div>";
        }

        $stmt->close();
    } else {
        $message = "<div class='error-message'><i class='fa-solid fa-circle-exclamation'></i> Hiba a lekérdezés előkészítésekor: " . $conn->error . "</div>";
    }
}
...
<head>
    ...
    <title>UMSZKI KRESZ - Regisztráció</title>
...
<body>
    ...
    <div class="register-container">
        ...
        <div class="register-header">
            ...
            <h1 class="register-title">UMSZKI KRESZ</h1>
            <p class="register-subtitle">Hozz létre egy új admin fiókot</p>
        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Add 3D hover effect to register container
            ...
            registerContainer.addEventListener('mousemove', (e) => {
                ...
                registerContainer.style.transform = `translateY(-5px) rotateX(${-moveY}deg) rotateY(${moveX}deg)`;
            });

            registerContainer.addEventListener('mouseleave', () => {
                registerContainer.style.transform = 'translateY(0) rotateX(0) rotateY(0)';
            });
        });
    </script>
</body>
